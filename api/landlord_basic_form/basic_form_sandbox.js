...
const templateTypeform = () => {
  ...
  axios.get(`https://api.typeform.com/forms/${basic_form_id}`, header)
    .then((data) => {
      console.log(data.data)
      const template = {
        form_id: basic_form_id,
      ...
      fs.writeFile(`./api/landlord_basic_form/${process.env.NODE_ENV}/templates/template_typeform_elastic_map.json`, JSON.stringify(template), function(err) {
        if(err) {
...
const templateDialogFlow = () => {
  ...
  axios.get(`https://dialogflow.googleapis.com/v2/projects/${PROJECT_ID}/agent/intents?pageSize=1000`, header)
    .then((data) => {
      ...
      fs.writeFile(`./api/landlord_basic_form/${process.env.NODE_ENV}/templates/template_elastic_dialog_map.json`, JSON.stringify(template), function(err) {
        if(err) {
            return console.log(err);
        }
