...
const testFormIdsMatch = () => {
  const p = new Promise((res, rej) => {
    console.log('=======> TEST 1: Testing that both our mappings have the same Typeform form_id (advanced_elastic_dialog_map and advanced_typeform_elastic_map)')
    console.log(`advanced_elastic_dialog_map.form_id = ${advanced_elastic_dialog_map.form_id}, advanced_typeform_elastic_map.form_id = ${advanced_typeform_elastic_map.form_id}, actual form_id = ${advanced_form_id}`)
    const result = (advanced_elastic_dialog_map.form_id === advanced_typeform_elastic_map.form_id) && (advanced_elastic_dialog_map.form_id === advanced_form_id)
    if (result) {
      console.log(`=======> TEST 1 PASSED: ${result}`)
      res(result)
    } else {
      console.log(`=======> TEST 1 FAILED: ${result}`)
      rej('The mappings you made did not have matching form_ids. Check advanced_typeform_elastic_map.js and advanced_elastic_dialog_map.js')
    }
  })
  return p
}
...
const testDialogIntentsExist = () => {
  console.log('=======> TEST 2: Testing that all Intents in our advanced_elastic_dialog_map match official DialogFlow intents')
  const headers = {
    headers: {
  ...
  const p = new Promise((res, rej) => {
    axios.get(`https://dialogflow.googleapis.com/v2/projects/${PROJECT_ID_LANDLORD_BACKEND}/agent/intents`, headers)
      .then((data) => {
        ...
        advanced_elastic_dialog_map.relationships.forEach((rel) => {
          let exists = false
...
const testAllIntentsMatchTypeformTags = () => {
  console.log('=======> TEST 3: Testing that all Intents in our advanced_elastic_dialog_map has an intent tag_id that matches advanced_typeform_elastic_map question tag_ids')
  let allMatched = true
  advanced_elastic_dialog_map.relationships.forEach((rel) => {
    let foundMatch = false
    advanced_typeform_elastic_map.questions.forEach((qs) => {
